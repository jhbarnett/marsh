...
2. **Inbox first**: run the `/marsh:inbox` contract — consume card replies
   ...
   everything else).
3. **Woken parks first**: `parked_tasks` rows with `woken_at` set are the
   highest-priority queue — their wait is over; route each to its station
   (or the digest if it needs the operator) and clear the row.
4. **Witness pass**: check `policy.witness.stuckThresholdHours` against
...
5. **Compute ready work per lane** (deterministically, per taxonomy):
...
6. **Dispatch within limits**: for each lane, up to `policy.lanes.<lane>.wip`
...
7. **Record**: one `station_passes` row per pass via
   `sh "${CLAUDE_PLUGIN_ROOT}/scripts/log_pass.sh"`; a `shift_log` row for
   the shift.
8. **Digest**: `reports/shift-<shift_id>.md` — dispatched (issue → station →
